...
class DDRAdam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8, weight_decay=0, degenerated_to_sgd=True):
        ...
        super(DDRAdam, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(DDRAdam, self).__setstate__(state)
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if N_sma >= 5:
                    ...
                    denom = (exp_avg_sq.add_(beta2, dv_t)).sqrt().add_(group['eps'])
                    p_data_fp32.addcdiv_(-step_size * group['lr'], exp_avg + beta1 * dm_t, denom)
                    p.data.copy_(p_data_fp32)
                elif step_size > 0:
                    if group['weight_decay'] != 0:
                        p_data_fp32.add_(-group['weight_decay'] * group['lr'], p_data_fp32)
                    p_data_fp32.add_(-step_size * group['lr'], exp_avg + beta1 * dm_t)
                    p.data.copy_(p_data_fp32)
