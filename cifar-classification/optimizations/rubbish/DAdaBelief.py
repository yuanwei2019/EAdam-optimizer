...
class DAdaBelief(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(DAdaBelief, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(DAdaBelief, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                grad_residual = grad - exp_avg - beta1 * dm_t
                exp_avg_var.mul_(beta2).addcmul_(1 - beta2, grad_residual, grad_residual)

                if 'd_lrp' not in state:
                    d_lrp = state['d_lrp'] = torch.zeros_like(grad)
                else:
                    d_lrp = state['d_lrp']
                d_crp = (grad_residual - d_lrp) ** 2
                state['d_lrp'] = grad_residual

                if 'dv_t' not in state:
                    dv_t = state['dv_t'] = torch.zeros_like(grad)
                else:
                    dv_t = state['dv_t']
                    dv_t.mul_(beta2).add_(1 - beta2, d_crp)
                ...
                if amsgrad:
                    ...
                    denom = ((max_exp_avg_var + beta2 * dv_t).sqrt() / math.sqrt(bias_correction2)).add_(group['eps'])
                else:
                    # denom = (exp_avg_var.add_(group['eps']).sqrt() / math.sqrt(bias_correction2)).add_(group['eps'])
                    denom = ((exp_avg_var.add_(group['eps']) + beta2 * dv_t).sqrt() / math.sqrt(bias_correction2)).add_(group['eps'])
                if not self.rectify:
                    # Default update
                    ...
                    if state['rho_t'] > 4:  # perform Adam style update if variance is small
                        ...
                        p.data.add_(-group['lr'], exp_avg + beta1 * dm_t)
