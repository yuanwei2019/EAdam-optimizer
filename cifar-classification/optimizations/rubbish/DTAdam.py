...
class DTAdam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(DTAdam, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(DTAdam, self).__setstate__(state)
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if amsgrad:
                    ...
                    denom = (exp_avg_sq.add_(beta2, dv_t).sqrt() / math.sqrt(bias_correction2)).add_(group['eps'])

                step_size = group['lr'] / bias_correction1

                p.data.addcdiv_(-step_size, exp_avg.add_(beta1, dm_t), denom)
